...
import re
from os.path import isdir, isfile, expanduser, split, isabs
from os.path import relpath, join, commonprefix, expandvars
...
def list_files(directory):
    """According to the given path, retrieving the files contained in the
    directory, otherwise return empy list."""
    f_list = []
...
def get_current_path():
    ...
    elif view is not None and view.file_name() is not None:
...
class Path_input():
    ...
    def update(self, text):
        if text.endswith(sep) and text != expandvars(text):
            self.set_text(convert_path(text))
        self.last_completion_failed = False
...
class iOpenerEventListener(sublime_plugin.EventListener):
    def on_query_context(self, view, key, operator, operand, match_all):
        return (key == 'i_opener' and iOpenerCommand.input_panel is not None
...
class iOpenerCompleteCommand(sublime_plugin.WindowCommand):
    ...
    def run(self):
        ...
        if input_panel.last_completion_failed:
            ...
            # Do path completion.
            path, status, complete = get_completion(
                convert_path(input_panel.get_text()))
